	...
	while (!dual_priqueue.empty()) {
		...
		if (queryNode->IsLeaf() || queryNode->Stat().isDense()) { //not descendable
			if (refNode->IsLeaf() || refNode->Stat().isDense()) {//both not descendable
				...
				if (queryNode == refNode && queryNode->Stat().isOnecluster()) continue;
				...
				//if (refNode->Right()->Begin() + refNode->Right()->Count() > queryNode->Stat().SearchStart()) {
				...
				//}
				//if (refNode->Left()->Begin() + refNode->Left()->Count() > queryNode->Stat().SearchStart()) {
			...
			if (queryNode == refNode) {
				if (queryNode->Stat().isOnecluster()) continue;
